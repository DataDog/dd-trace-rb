...
class RubyOverheadExperiment
  ...
  def workload(steps_per_sample:, loop_times:, profile:, sync_queue:, finished_queue:)
    sync_queue.pop
    ...
    finished_queue << true
    sleep # Keep thread alive at end
  end
  ...
  def simulate(**config)
    ...
    #30.times do
      ...
      launched_threads = threads.times.map do
        Thread.new { workload(steps_per_sample: 10, loop_times: seconds, profile: profiler_enabled, sync_queue: sync_queue, finished_queue: finished_queue) }
      end

      threads.times { sync_queue << true }
      threads.times { finished_queue.pop }

      launched_threads.each(&:kill).each(&:join)
