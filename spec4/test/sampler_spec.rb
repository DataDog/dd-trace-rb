...
RSpec.describe Datadog::RateSampler do
  ...
  it('rate sampler invalid') do
    sampler = described_class.new(-1.0)
    expect(sampler.sample_rate).to(eq(1.0))
    sampler = described_class.new(0.0)
    expect(sampler.sample_rate).to(eq(1.0))
    sampler = described_class.new(1.5)
    expect(sampler.sample_rate).to(eq(1.0))
  end
  it('rate sampler 1 0') do
    ...
    spans.each do |span|
      sampler.sample(span)
      expect(span.sampled).to(eq(true))
      expect(span.get_metric(described_class::SAMPLE_RATE_METRIC_KEY)).to(eq(1.0))
    end
  end
  it('rate sampler') do
    prng = Random.new(123)
    [0.1, 0.25, 0.5, 0.9].each do |sample_rate|
      ...
      sampler = described_class.new(sample_rate)
      spans.each do |span|
        sampler.sample(span)
        if span.sampled
          expect(span.get_metric(described_class::SAMPLE_RATE_METRIC_KEY)).to(eq(sample_rate))
        end
      end
  ...
  it('tracer with rate sampler') do
    ...
    tracer.configure(sampler: described_class.new(0.5))
    nb_spans = 10000
    nb_spans.times do
    ...
    spans.each do |span|
      expect(span.get_metric(described_class::SAMPLE_RATE_METRIC_KEY)).to(eq(0.5))
    end
  end
end
