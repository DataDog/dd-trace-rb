...
if %w[1 true yes].include?(Datadog.get_environment_variable('DD_DYNAMIC_INSTRUMENTATION_ENABLED')) # steep:ignore
...
if %w[1 true yes].include?(Datadog.get_environment_variable('DD_DYNAMIC_INSTRUMENTATION_ENABLED')) # steep:ignore
  if Datadog.get_environment_variable('DD_DYNAMIC_INSTRUMENTATION_PROBE_FILE')
