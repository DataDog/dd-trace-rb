      ...
      class Worker
        ...
        def started!
          ...
          initial_event_once.run do
            ...
            if res.ok?
              logger.debug { "Telemetry initial event (#{initial_event.class}) is successfully sent" }
              ...
              logger.debug("Error sending telemetry initial event (#{initial_event.class}), retry after heartbeat interval...")
              false
            end
          end

          if failed_initial_event?
            logger.debug { "Telemetry initial event (#{initial_event.class}) exhausted retries, disabling telemetry worker" }
            disable!
          end
        end
