  ...
  module Tracing
    ...
    class TraceSegment
      ...
      def initialize(
        ...
        hostname && !hostname.frozen? ? hostname.dup : hostname

        self.agent_sample_rate = agent_sample_rate
        self.hostname = (hostname && !hostname.frozen? ? hostname.dup.freeze : hostname)
        self.lang = (lang && !lang.frozen? ? lang.dup.freeze : lang)
        self.name = (name && !name.frozen? ? name.dup.freeze : name)
        self.origin = (origin && !origin.frozen? ? origin.dup.freeze : origin)
        self.process_id = process_id
        self.rate_limiter_rate = rate_limiter_rate
        self.resource = (resource && !resource.frozen? ? resource.dup.freeze : resource)
        self.rule_sample_rate = rule_sample_rate
        self.runtime_id = (runtime_id && !runtime_id.frozen? ? runtime_id.dup.freeze : runtime_id)
        self.sample_rate = sample_rate
        self.sampling_priority = sampling_priority
        self.service = (service && !service.frozen? ? service.dup.freeze : service)
      end
