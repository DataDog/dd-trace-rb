...
class LoggerTest < Minitest::Test
  ...
  def test_tracer_logger_override
    ...
    lines.each do |l|
      ...
      when 1
        assert_match(/E,.*ERROR -- ddtrace: \[ddtrace\] \(.*logger_test.rb\:.*test_tracer_logger_override.*\) this does not work/, l)
      when 2
        assert_match(/E,.*ERROR -- foo: \[ddtrace\] \(.*logger_test.rb\:.*test_tracer_logger_override.*\) neither does this/, l)
      when 3
