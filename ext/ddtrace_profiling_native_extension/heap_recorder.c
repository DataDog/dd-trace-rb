...
void heap_recorder_for_each_live_object(
    ...
    bool with_gvl) {
  if (!with_gvl) {
    // Despite our read-only access to object_records, we are outside the GVL so may race
    // with the GVL-holding writers and lead to mutations-during-iterations.
...
static int st_heap_record_entry_free(st_data_t key, st_data_t value, st_data_t extra_arg) {
  ...
  if (record_key->type != HEAP_STACK) {
    rb_raise(rb_eRuntimeError, "Used something other than a heap stack as heap_records key");
  }
