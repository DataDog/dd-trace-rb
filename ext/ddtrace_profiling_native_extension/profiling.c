...
static int borrowed_from_ruby_sources_rb_profile_frames(rb_execution_context_t *ec, int start, int limit, VALUE *buff, int *lines);
...
static VALUE sample_thread(VALUE thread) {
  ...
  // FIXME:
  // 2. Need to convert the data to something interesting
  int stack_depth = borrowed_from_ruby_sources_rb_profile_frames(thread_pointer->ec, 0, MAX_STACK_DEPTH, frames, lines);

  return rb_ary_new_from_args(2, thread, INT2FIX(stack_depth));
}
