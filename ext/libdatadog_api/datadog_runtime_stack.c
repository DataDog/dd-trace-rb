...
static inline bool is_pointer_readable(const void *ptr, size_t size) {
  ...
  // Number of pages spanned (1 or more)
...
VALUE datadog_runtime_stack_register_callback(VALUE callback_type) {
  return _native_register_runtime_stack_callback(Qnil, callback_type);
}
