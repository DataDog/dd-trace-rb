...
static bool build_and_send_crash_report(ddog_crasht_Handle_CrashInfoBuilder *builder,
...
static VALUE _native_report_ruby_exception(DDTRACE_UNUSED VALUE _self, VALUE agent_base_url,
  ...
  bool success = build_and_send_crash_report(builder, metadata, endpoint, message, frames_data);
...
static bool build_and_send_crash_report(ddog_crasht_Handle_CrashInfoBuilder *builder,
  ...
  do {
    if (ddog_crasht_CrashInfoBuilder_with_metadata(builder, metadata).tag != DDOG_VOID_RESULT_OK) break;
    // Send ping first
    if (ddog_crasht_CrashInfoBuilder_upload_ping_to_endpoint(builder, endpoint).tag != DDOG_VOID_RESULT_OK) break;

    if (ddog_crasht_CrashInfoBuilder_with_kind(builder, DDOG_CRASHT_ERROR_KIND_UNHANDLED_EXCEPTION).tag != DDOG_VOID_RESULT_OK) break;
    if (ddog_crasht_CrashInfoBuilder_with_timestamp_now(builder).tag != DDOG_VOID_RESULT_OK) break;

    ddog_crasht_ProcInfo proc_info = { .pid = (uint32_t)getpid() };
    if (ddog_crasht_CrashInfoBuilder_with_proc_info(builder, proc_info).tag != DDOG_VOID_RESULT_OK) break;
    if (ddog_crasht_CrashInfoBuilder_with_os_info_this_machine(builder).tag != DDOG_VOID_RESULT_OK) break;

    if (ddog_crasht_CrashInfoBuilder_with_message(builder, char_slice_from_ruby_string(message)).tag != DDOG_VOID_RESULT_OK) break;

    ddog_crasht_StackTrace_NewResult stack_result = ddog_crasht_StackTrace_new();
    if (stack_result.tag != DDOG_CRASHT_STACK_TRACE_NEW_RESULT_OK) break;

    stack_trace = &stack_result.ok;

    if (!process_crash_frames(frames_data, stack_trace)) break;

    if (ddog_crasht_StackTrace_set_complete(stack_trace).tag != DDOG_VOID_RESULT_OK) break;
    if (ddog_crasht_CrashInfoBuilder_with_stack(builder, stack_trace).tag != DDOG_VOID_RESULT_OK) break;

    // Builder takes ownership of stack_trace, so reset to NULL to avoid double cleanup
    stack_trace = NULL;
    ...
    if (crash_info_result.tag == DDOG_CRASHT_RESULT_HANDLE_CRASH_INFO_OK_HANDLE_CRASH_INFO) {
      ...
      if (upload_result.tag == DDOG_VOID_RESULT_OK) {
        success = true;
      }
  ...
  // cleanup - stack_trace must be cleaned up if we broke early before giving it to builder
  if (stack_trace != NULL) {
