...
//
// When a qualified name (e.g. `Foo::Bar#baz`) is built, it is written into `buf`, `*name_slice` is updated to point at
// it, and the number of bytes taken from `buf` is returned. Otherwise `*name_slice` is left as-is (the bare method
// name) and 0 is returned, so the caller can advance its buffer position by simply adding the return value.
static size_t build_qualified_name(VALUE defined_class, ddog_CharSlice *name_slice, char *buf, size_t buf_size) {
  ...
  *name_slice = (ddog_CharSlice) {.ptr = buf, .len = total};
  return total;
}
