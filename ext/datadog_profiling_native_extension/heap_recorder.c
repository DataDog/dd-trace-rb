...
void start_heap_allocation_recording(heap_recorder *heap_recorder, VALUE new_obj, unsigned int weight, ddog_CharSlice alloc_class) {
  ...
  if (heap_recorder->active_recording != NULL) {
    raise_for_telemetry("Detected consecutive heap allocation recording starts without end.");
  }
  ...
  if (!FIXNUM_P(ruby_obj_id)) {
    raise_for_telemetry("Detected a bignum object id. These are not supported by heap profiling.");
  }
...
static VALUE end_heap_allocation_recording(VALUE protect_args) {
  ...
  if (active_recording == NULL) {
    ...
    raise_for_telemetry("Ended a heap recording that was not started");
  }
...
void heap_recorder_prepare_iteration(heap_recorder *heap_recorder) {
  ...
  if (heap_recorder->object_records_snapshot != NULL) {
    ...
    raise_for_telemetry("New heap recorder iteration prepared without the previous one having been finished.");
  }
  ...
  if (heap_recorder->object_records_snapshot == NULL) {
    raise_for_telemetry("Failed to create heap snapshot.");
  }
}
...
void heap_recorder_finish_iteration(heap_recorder *heap_recorder) {
  ...
  if (heap_recorder->object_records_snapshot == NULL) {
    // we could trivially handle this but we raise to highlight and catch unexpected usages.
    raise_for_telemetry("Heap recorder iteration finished without having been prepared.");
  }
...
static void commit_recording(heap_recorder *heap_recorder, heap_record *heap_record, object_record *active_recording) {
  ...
  if (heap_record->num_tracked_objects == UINT32_MAX) {
    raise_for_telemetry("Reached maximum number of tracked objects for heap record");
  }
  ...
  if (existing_error) {
    ...
    if (existing_record == NULL) raise_for_telemetry("Unexpected NULL when reading existing record");
    ...
    raise_for_telemetry("Object ids are supposed to be unique. We got 2 allocation recordings with "
...
static void cleanup_heap_record_if_unused(heap_recorder *heap_recorder, heap_record *heap_record) {
  ...
  if (!st_delete(heap_recorder->heap_records, (st_data_t*) &heap_record, NULL)) {
    raise_for_telemetry("Attempted to cleanup an untracked heap_record");
  };
...
static void on_committed_object_record_cleanup(heap_recorder *heap_recorder, object_record *record) {
  ...
  if (heap_recorder == NULL) raise_for_telemetry("heap_recorder was NULL in on_committed_object_record_cleanup");
  if (heap_recorder->heap_records == NULL) raise_for_telemetry("heap_recorder->heap_records was NULL in on_committed_object_record_cleanup");
  if (record == NULL) raise_for_telemetry("record was NULL in on_committed_object_record_cleanup");
  ...
  if (heap_record == NULL) raise_for_telemetry("heap_record was NULL in on_committed_object_record_cleanup");
...
heap_record* heap_record_new(heap_recorder *recorder, ddog_prof_Slice_Location locations) {
  ...
  if (frames_len > MAX_FRAMES_LIMIT) {
    ...
    raise_for_telemetry("Found stack with more than %d frames (%d)", MAX_FRAMES_LIMIT, frames_len);
  }
...
static void unintern_or_raise(heap_recorder *recorder, ddog_prof_ManagedStringId id) {
  ...
  if (result.tag == DDOG_PROF_OPTION_ERROR_SOME_ERROR) {
    raise_for_telemetry("Failed to unintern id: %"PRIsVALUE, get_error_details_and_drop(&result.some));
  }
}
...
static void unintern_all_or_raise(heap_recorder *recorder, ddog_prof_Slice_ManagedStringId ids) {
  ...
  if (result.tag == DDOG_PROF_OPTION_ERROR_SOME_ERROR) {
    raise_for_telemetry("Failed to unintern_all: %"PRIsVALUE, get_error_details_and_drop(&result.some));
  }
}
...
static VALUE get_ruby_string_or_raise(heap_recorder *recorder, ddog_prof_ManagedStringId id) {
  ...
  if (get_string_result.tag == DDOG_STRING_WRAPPER_RESULT_ERR) {
    raise_for_telemetry("Failed to get string: %"PRIsVALUE, get_error_details_and_drop(&get_string_result.err));
  }
