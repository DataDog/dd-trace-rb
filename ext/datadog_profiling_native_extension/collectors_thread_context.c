...
static void stats_to_ruby_hash(thread_context_collector_state *state, VALUE hash);
...
static bool skip_sample(thread_context_collector_state *state, per_thread_context *thread_context, bool is_gvl_waiting_state, bool force_sample_suspended) {
  ...
  // be picked up by either the next regular sample after the thread acquires the GVL, or by
...
static VALUE _native_inspect(DDTRACE_UNUSED VALUE _self, VALUE collector_instance) {
  ...
  rb_str_concat(result, rb_sprintf(" stats=%"PRIsVALUE, stats_hash));
...
void thread_context_collector_stats_reset_not_thread_safe(VALUE self_instance) {
  ...
  // just set the low bit to SUSPENDED; the sampler compares it against its snapshot
  ...
  on_gvl_running_result thread_context_collector_on_gvl_running(VALUE self_instance, VALUE thread, per_thread_context *thread_context) {
    ...
    if (should_sample) {
      ...
      if (thread_context->was_skipped_at_last_sample) {
        ...
        // and we want to attribute the time without the GVL to the correct Ruby stack
        // (i.e. the correct cfunc method which releases the GVL).
