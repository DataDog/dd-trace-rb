...
ddog_prof_ManagedStringId intern_or_raise(ddog_prof_ManagedStringStorage string_storage, ddog_CharSlice string) {
  ...
  if (intern_result.tag == DDOG_PROF_MANAGED_STRING_STORAGE_INTERN_RESULT_ERR) {
    raise_error(eNativeRuntimeError, "Failed to intern string: %"PRIsVALUE, get_error_details_and_drop(&intern_result.err));
  }
...
void intern_all_or_raise(
  ...
  if (result.tag == DDOG_PROF_OPTION_ERROR_SOME_ERROR) {
    raise_error(eNativeRuntimeError, "Failed to intern_all: %"PRIsVALUE, get_error_details_and_drop(&result.some));
  }
}
