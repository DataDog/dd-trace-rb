...
#define CPU_TIME_VALUE          DDOG_PROF_SAMPLE_TYPE_CPU_TIME
#define CPU_TIME_VALUE_ID 0
#define CPU_SAMPLES_VALUE       DDOG_PROF_SAMPLE_TYPE_CPU_SAMPLES
#define CPU_SAMPLES_VALUE_ID 1
#define WALL_TIME_VALUE         DDOG_PROF_SAMPLE_TYPE_WALL_TIME
#define WALL_TIME_VALUE_ID 2
#define ALLOC_SAMPLES_VALUE     DDOG_PROF_SAMPLE_TYPE_ALLOC_SAMPLES
#define ALLOC_SAMPLES_VALUE_ID 3
#define ALLOC_SAMPLES_UNSCALED_VALUE DDOG_PROF_SAMPLE_TYPE_ALLOC_SAMPLES_UNSCALED
#define ALLOC_SAMPLES_UNSCALED_VALUE_ID 4
#define HEAP_SAMPLES_VALUE      DDOG_PROF_SAMPLE_TYPE_HEAP_LIVE_SAMPLES
#define HEAP_SAMPLES_VALUE_ID 5
#define HEAP_SIZE_VALUE         DDOG_PROF_SAMPLE_TYPE_HEAP_LIVE_SIZE
#define HEAP_SIZE_VALUE_ID 6
#define TIMELINE_VALUE          DDOG_PROF_SAMPLE_TYPE_TIMELINE
#define TIMELINE_VALUE_ID 7

static const ddog_prof_SampleType all_value_types[] =
...
#define ALL_VALUE_TYPES_COUNT (sizeof(all_value_types) / sizeof(ddog_prof_SampleType))
...
static VALUE _native_new(VALUE klass) {
  ...
  ddog_prof_Slice_SampleType sample_types = {.ptr = all_value_types, .len = ALL_VALUE_TYPES_COUNT};
...
static VALUE _native_initialize(int argc, VALUE *argv, DDTRACE_UNUSED VALUE _self) {
  ...
  ddog_prof_SampleType enabled_value_types[ALL_VALUE_TYPES_COUNT];
  uint8_t next_enabled_pos = 0;
  uint8_t next_disabled_pos = requested_values_count;

  // CPU_SAMPLES_VALUE is always enabled
  enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) CPU_SAMPLES_VALUE;
  state->position_for[CPU_SAMPLES_VALUE_ID] = next_enabled_pos++;

  // WALL_TIME_VALUE is always enabled
  enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) WALL_TIME_VALUE;
  state->position_for[WALL_TIME_VALUE_ID] = next_enabled_pos++;

  if (cpu_time_enabled == Qtrue) {
    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) CPU_TIME_VALUE;
  ...
  if (alloc_samples_enabled == Qtrue) {
    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) ALLOC_SAMPLES_VALUE;
    state->position_for[ALLOC_SAMPLES_VALUE_ID] = next_enabled_pos++;

    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) ALLOC_SAMPLES_UNSCALED_VALUE;
  ...
  if (heap_samples_enabled == Qtrue) {
    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) HEAP_SAMPLES_VALUE;
  ...
  if (heap_size_enabled == Qtrue) {
    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) HEAP_SIZE_VALUE;
  ...
  if (timeline_enabled == Qtrue) {
    enabled_value_types[next_enabled_pos] = (ddog_prof_SampleType) TIMELINE_VALUE;
  ...
  ddog_prof_Slice_SampleType sample_types = {.ptr = enabled_value_types, .len = state->enabled_values_count};
  initialize_profiles(state, sample_types);
