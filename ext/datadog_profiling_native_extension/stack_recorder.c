...
static VALUE _native_new(VALUE klass) {
  ...
  if (string_storage.tag == DDOG_PROF_MANAGED_STRING_STORAGE_NEW_RESULT_ERR) {
    raise_error(eDatadogRuntimeError, "Failed to initialize string storage: %"PRIsVALUE, get_error_details_and_drop(&string_storage.err));
  }
...
static void initialize_profiles(stack_recorder_state *state, ddog_prof_Slice_ValueType sample_types) {
  ...
  if (slot_one_profile_result.tag == DDOG_PROF_PROFILE_NEW_RESULT_ERR) {
    raise_error(eDatadogRuntimeError, "Failed to initialize slot one profile: %"PRIsVALUE, get_error_details_and_drop(&slot_one_profile_result.err));
  }
  ...
  if (slot_two_profile_result.tag == DDOG_PROF_PROFILE_NEW_RESULT_ERR) {
    ...
    raise_error(eDatadogRuntimeError, "Failed to initialize slot two profile: %"PRIsVALUE, get_error_details_and_drop(&slot_two_profile_result.err));
  }
...
static VALUE _native_serialize(DDTRACE_UNUSED VALUE _self, VALUE recorder_instance) {
  ...
  if (result.tag == DDOG_PROF_OPTION_ERROR_SOME_ERROR) {
    raise_error(eDatadogRuntimeError, "Failed to advance string storage gen: %"PRIsVALUE, get_error_details_and_drop(&result.some));
  }
...
void record_sample(VALUE recorder_instance, ddog_prof_Slice_Location locations, sample_values values, sample_labels labels) {
  ...
  if (result.tag == DDOG_PROF_PROFILE_RESULT_ERR) {
    raise_error(eDatadogArgumentError, "Failed to record sample: %"PRIsVALUE, get_error_details_and_drop(&result.err));
  }
}
...
void record_endpoint(VALUE recorder_instance, uint64_t local_root_span_id, ddog_CharSlice endpoint) {
  ...
  if (result.tag == DDOG_PROF_PROFILE_RESULT_ERR) {
    raise_error(eDatadogArgumentError, "Failed to record endpoint: %"PRIsVALUE, get_error_details_and_drop(&result.err));
  }
}
...
static void build_heap_profile_without_gvl(stack_recorder_state *state, profile_slot *slot) {
  ...
  if (!iterated) {
    grab_gvl_and_raise(eDatadogRuntimeError, "Failure during heap profile building: iteration cancelled");
  }
  else if (iteration_context.error) {
    grab_gvl_and_raise(eDatadogRuntimeError, "Failure during heap profile building: %s", iteration_context.error_msg);
  }
}
...
static locked_profile_slot sampler_lock_active_profile(stack_recorder_state *state) {
  ...
  raise_error(eDatadogRuntimeError, "Failed to grab either mutex in sampler_lock_active_profile");
}
...
static profile_slot* serializer_flip_active_and_inactive_slots(stack_recorder_state *state) {
  ...
  if (previously_active_slot != 1 && previously_active_slot != 2) {
    grab_gvl_and_raise(eDatadogRuntimeError, "Unexpected active_slot state %d in serializer_flip_active_and_inactive_slots", previously_active_slot);
  }
...
static VALUE test_slot_mutex_state(VALUE recorder_instance, int slot) {
  ...
  if (error == 0) {
    ...
    raise_error(eDatadogRuntimeError, "Failed to raise exception in test_slot_mutex_state; this should never happen");
  }
}
...
static void reset_profile_slot(profile_slot *slot, ddog_Timespec start_timestamp) {
  ...
  if (reset_result.tag == DDOG_PROF_PROFILE_RESULT_ERR) {
    raise_error(eDatadogRuntimeError, "Failed to reset profile: %"PRIsVALUE, get_error_details_and_drop(&reset_result.err));
  }
...
static VALUE _native_test_managed_string_storage_produces_valid_profiles(DDTRACE_UNUSED VALUE _self) {
  ...
  if (profile.tag == DDOG_PROF_PROFILE_NEW_RESULT_ERR) {
    raise_error(eDatadogRuntimeError, "Failed to initialize profile: %"PRIsVALUE, get_error_details_and_drop(&profile.err));
  }
  ...
  if (serialize_result.tag == DDOG_PROF_PROFILE_SERIALIZE_RESULT_ERR) {
    raise_error(eDatadogRuntimeError, "Failed to serialize: %"PRIsVALUE, get_error_details_and_drop(&serialize_result.err));
  }
  ...
  if (advance_gen_result.tag == DDOG_PROF_OPTION_ERROR_SOME_ERROR) {
    raise_error(eDatadogRuntimeError, "Failed to advance string storage gen: %"PRIsVALUE, get_error_details_and_drop(&advance_gen_result.some));
  }
  ...
  if (serialize_result.tag == DDOG_PROF_PROFILE_SERIALIZE_RESULT_ERR) {
    raise_error(eDatadogArgumentError, "Failed to serialize: %"PRIsVALUE, get_error_details_and_drop(&serialize_result.err));
  }
