# Ruby: Demo Ruby+PG application for Datadog APM

A Ruby application set up with a PostgreSQL database.

For generating Datadog APM traces with the `pg` gem.
...
Within the container, run `bin/run <process>` where `<process>` is one of the following values:
 ...
 - `pg` : Sends random queries to the PostgreSQL database using the `pg` gem
 - `irb`: IRB session
