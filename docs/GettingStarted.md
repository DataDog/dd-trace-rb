...
| `template_base_path` | `DD_TRACE_ACTION_VIEW_TEMPLATE_BASE_PATH` | `String` | Used when the template name is parsed. If you don't store your templates in the `views/` folder, you may need to change this value | `'views/'` |
...
| `service_name` | `DD_TRACE_ACTIVE_JOB_SERVICE_NAME` | `String` | Service name reported by `active_job` spans. | `'active_job'` |
...
| `service_name` | `DD_TRACE_ACTIVE_RECORD_SERVICE_NAME` | `String` | Override the service name for the SQL query instrumentation. ActiveRecord instantiation instrumentation always uses the application's configured service name. | Name of database adapter (e.g. `'mysql2'`) |
...
| `cache_key.enabled` | `DD_TRACE_ACTIVE_SUPPORT_CACHE_KEY_ENABLED` | `Bool` | Enable inclusion of the cache key on cache spans. | `true` |
| `cache_service` | `DD_TRACE_ACTIVE_SUPPORT_CACHE_SERVICE` | `String` | Name of application running the `active_support` instrumentation. May be overridden by `global_default_service_name`. [See _Additional Configuration_ for more details](#additional-configuration) | `active_support-cache` |
| `cache_store` | `DD_TRACE_ACTIVE_SUPPORT_CACHE_STORE` | `Array` | Specifies which cache stores to instrument. Accepts a list of store names (e.g. `memory_store`, `file_store`, or symbols like `:file_store`). If set, only the listed stores will be traced. By default (`nil`), it traces all stores. | `nil` |
...
| `quantize`     | `DD_TRACE_ELASTICSEARCH_QUANTIZE` | `JSON`   | Hash containing options for quantization. May include `:show` with an Array of keys to not quantize (or `:all` to skip quantization), or `:exclude` with Array of keys to exclude entirely. Provide a JSON object when setting this through the environment.       | `{}`            |
...
| `split_by_domain`     | `DD_TRACE_ETHON_SPLIT_BY_DOMAIN` | `Bool`   | Uses the request domain as the service name when set to `true`.                                                                                                                           | `false` |
...
| `split_by_domain`     | `DD_TRACE_EXCON_SPLIT_BY_DOMAIN` | `Bool`           | Uses the request domain as the service name when set to `true`.                                                                                                                                                                                                                    | `false`                                                           |
...
| `split_by_domain`     | `DD_TRACE_FARADAY_SPLIT_BY_DOMAIN` | `Bool`           | Uses the request domain as the service name when set to `true`.                                                                                                                                                                                                                    | `false`                                                           |
...
| `schemas`                | `DD_TRACE_GRAPHQL_SCHEMAS` | `Array`  | Array of `GraphQL::Schema` objects (that support class-based schema only) to trace. If you do not provide any, then tracing will applied to all the schemas.                                                    | `[]`             |
...
| `with_deprecated_tracer` | `DD_TRACE_GRAPHQL_WITH_DEPRECATED_TRACER` | `Bool`   | (Not recommended) Enable to instrument with deprecated `GraphQL::Tracing::DataDogTracing`. This has priority over `with_unified_tracer`. Default is `false`, using `GraphQL::Tracing::DataDogTrace` instead | `false` |
| `service_name`           | `DD_TRACE_GRAPHQL_SERVICE_NAME` | `String` | Service name used for graphql instrumentation                                                                                                                                                                   | `'ruby-graphql'` |
...
| `service_name` | `DD_TRACE_HANAMI_SERVICE_NAME` | `String` | Service name for `hanami` instrumentation. | `nil`   |
...
| `split_by_domain`     | `DD_TRACE_HTTPRB_SPLIT_BY_DOMAIN` | `Bool`           | Uses the request domain as the service name when set to `true`.                                                                                                                                                                                                                        | `false`     |
...
| `split_by_domain`     | `DD_TRACE_HTTPCLIENT_SPLIT_BY_DOMAIN` | `Bool`           | Uses the request domain as the service name when set to `true`.                                                                                                                                                                                                                    | `false`      |
...
| `tag_body` | `DD_TRACE_SNEAKERS_TAG_BODY` | `Bool` | Whether to tag the job message on spans.                                                                                                                 | `false`                                                           |
...
| `quantize`     | `DD_TRACE_MONGO_QUANTIZE` | `JSON`   | Hash containing options for quantization. May include `:show` with an Array of keys to not quantize (or `:all` to skip quantization), or `:exclude` with Array of keys to exclude entirely. Provide a JSON object when setting this through the environment. | `{ show: [:collection, :database, :operation] }` |
...
| `split_by_domain`     | `DD_TRACE_HTTP_SPLIT_BY_DOMAIN` | `Bool`           | Uses the request domain as the service name when set to `true`.                                                                                                                                                                                                                    | `false`     |
...
| `quantize` | `DD_TRACE_OPENSEARCH_QUANTIZE` | `JSON` | Hash containing options for quantization. May include `:show` with an Array of keys to not quantize, `:all` to skip quantization, or `:exclude` with an Array of keys to exclude entirely. Provide a JSON object when setting this through the environment. | `{}` |
...
| `service_name`                  | `DD_TRACE_RACK_SERVICE_NAME` | `String` | Service name used for Rack request spans.                                                                                                                                                                                                                                                                                                                                                | `nil`                                            |
| `headers`                        | `DD_TRACE_RACK_HEADERS` | `JSON`   | Hash of HTTP request or response headers to add as tags to the `rack.request`. Accepts `request` and `response` keys with Array values e.g. `['Last-Modified']`. Adds `http.request.headers.*` and `http.response.headers.*` tags respectively. This option overrides the global `DD_TRACE_HEADER_TAGS`, see [Applying header tags to root spans][header tags] for more information. Provide a JSON object when setting this through the environment. | `{ response: ['Content-Type', 'X-Request-ID'] }` |
| `middleware_names`               | `DD_TRACE_RACK_MIDDLEWARE_NAMES` | `Bool`   | Enable this if you want to use the last executed middleware class as the resource name for the `rack` span. If enabled alongside the `rails` instrumention, `rails` takes precedence by setting the `rack` resource name to the active `rails` controller when applicable. Requires `application` option to use.                                                                     | `false`                                          |
| `quantize`                       | `DD_TRACE_RACK_QUANTIZE` | `JSON`   | Hash containing options for quantization. May include `:query` or `:fragment`. Provide a JSON object when setting this through the environment.                                                                                                                                                                                                                                                                                                       | `{}`                                             |
...
| `request_queuing`                | `DD_TRACE_RACK_REQUEST_QUEUING` | `Bool`   | Track HTTP request time spent in the queue of the frontend server. See [HTTP request queuing](#http-request-queuing) for setup details.                                                                                                                                                                                                                                             | `false`                                          |
| `web_service_name`               | `DD_TRACE_RACK_WEB_SERVICE_NAME` | `String` | Service name for frontend server request queuing spans. (e.g. `'nginx'`)                                                                                                                                                                                                                                                                                                             | `'web-server'`                                   |
...
| `request_queuing`     | `DD_TRACE_RAILS_REQUEST_QUEUING` | `Bool`   | Track HTTP request time spent in the queue of the frontend server. See [HTTP request queuing](#http-request-queuing) for setup details.                     | `false`                                                         |
| `middleware`          | `DD_TRACE_RAILS_MIDDLEWARE` | `Bool`   | Add the trace middleware to the Rails application. Set to `false` if you don't want the middleware to load.                                                 | `true`                                                          |
| `middleware_names`    | `DD_TRACE_RAILS_MIDDLEWARE_NAMES` | `Bool`   | Enables any short-circuited middleware requests to display the middleware name as a resource for the trace.                                                 | `false`                                                         |
| `service_name`        | `DD_TRACE_RAILS_SERVICE_NAME` | `String` | Service name used when tracing application requests (on the `rack` level)                                                                                   | `'<app_name>'` (inferred from your Rails application namespace) |
| `template_base_path`  | `DD_TRACE_RAILS_TEMPLATE_BASE_PATH` | `String` | Used when the template name is parsed. If you don't store your templates in the `views/` folder, you may need to change this value                          | `'views/'`                                                      |
...
| `quantize`     | `DD_TRACE_RAKE_QUANTIZE` | `JSON`   | Hash containing options for quantization of task arguments. See below for more details and examples. Provide a JSON object when setting this through the environment.     | `{}`     |
| `service_name` | `DD_TRACE_RAKE_SERVICE_NAME` | `String` | Service name used for `rake` instrumentation                                                             | `'rake'` |
...
| `split_by_domain`     | `DD_TRACE_REST_CLIENT_SPLIT_BY_DOMAIN` | `Bool`   | Uses the request domain as the service name when set to `true`.                                                                                                                                 | `false`       |
...
| `service_name` | `DD_TRACE_RODA_SERVICE_NAME` | `String` | Service name for `roda` instrumentation. | `nil`   |
...
| `service_name` | `DD_TRACE_SEQUEL_SERVICE_NAME` | `String` | Service name for `sequel` instrumentation | Name of database adapter (e.g. `'mysql2'`) |
...
| `tag_body` | `DD_TRACE_SHORYUKEN_TAG_BODY` | `Bool` | Tag spans with the SQS message body `true` or `false`                                                                                                                       | `false`                                                           |
| `on_error` | | `Proc` | Custom error handler invoked when a job raises an error. Provided `span` and `error` as arguments. Sets error on the span by default. Useful for ignoring transient errors. | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
...
| `service_name` | `DD_TRACE_SIDEKIQ_SERVICE_NAME` | `String` | Service name used for server-side Sidekiq spans. | `nil` |
| `client_service_name` | `DD_TRACE_SIDEKIQ_CLIENT_SERVICE_NAME` | `String` | Service name used for client-side Sidekiq enqueue spans. | `nil` |
| `on_error`            | | `Proc` | Custom error handler invoked when a job raises an error. Provided `span` and `error` as arguments. Sets error on the span by default. Useful for ignoring transient errors.                                                                                                                                                                                                                                                                           | `proc { \|span, error\| span.set_error(error) unless span.nil? }` |
| `quantize`            | `DD_TRACE_SIDEKIQ_QUANTIZE` | `JSON` | Hash containing options for quantization of job arguments. Provide a JSON object when setting this through the environment.                                                                                                                                                                                                                                                                                                                                                                                            | `{}`                                                              |
...
| `headers`               | `DD_TRACE_SINATRA_HEADERS` | `JSON` | Hash of HTTP request or response headers to add as tags to the `sinatra.request`. Accepts `request` and `response` keys with Array values e.g. `['Last-Modified']`. Adds `http.request.headers.*` and `http.response.headers.*` tags respectively. This option overrides the global `DD_TRACE_HEADER_TAGS`, see [Applying header tags to root spans][header tags] for more information. Provide a JSON object when setting this through the environment. | `{ response: ['Content-Type', 'X-Request-ID'] }` |
| `resource_script_names` | `DD_TRACE_SINATRA_RESOURCE_SCRIPT_NAMES` | `Bool` | Prepend resource names with script name                                                                                                                                                                                                                                                                                                                                                 | `false`                                          |
| `service_name` | `DD_TRACE_SINATRA_SERVICE_NAME` | `String` | Service name for `sinatra` instrumentation. | `nil` |
...
| `service_name` | `DD_TRACE_SUCKER_PUNCH_SERVICE_NAME` | `String` | Service name reported by `sucker_punch` spans. | `'sucker_punch'` |
...
| `tracing.partial_flush.enabled`                        | `DD_TRACE_PARTIAL_FLUSH_ENABLED`                        | `Bool`                                | `false`                      | Enables or disables partial flushing. Partial flushing submits completed portions of a trace to the agent. Used when tracing instruments long running tasks (e.g. jobs) with many spans.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `tracing.partial_flush.min_spans_threshold`            | `DD_TRACE_PARTIAL_FLUSH_MIN_SPANS`                      | `Integer`                             | `500`                        | The number of spans that must be completed in a trace before partial flushing submits those completed spans.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
You can also control the default logger severity with `DD_TRACE_LOG_LEVEL` using names like `debug`, `info`, `warn`, `error`, `fatal`, or `unknown`.
