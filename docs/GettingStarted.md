...
| `comment_propagation` | `DD_DBM_PROPAGATION_MODE`      | `String` | SQL comment propagation mode for database monitoring. <br />(example: `disabled` \| `service`\| `dynamic_service` \| `full`). <br /><br />**Important**: _Enabling SQL comment propagation may store potentially confidential data (service names) in the databases, which can then be accessed by other third parties that have been granted database access._ | `'disabled'`                                                      |
