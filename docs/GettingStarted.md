...
If your application either receives or sends requests through non-instrumented code, you must manually configure distributed tracing.

Spans on receiving applications must be linked to their parent trace by setting `trace_id` and `parent_id`:
