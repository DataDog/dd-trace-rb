...
| `tracing.otel_thread_context_enabled`                  | `DD_TRACE_OTEL_CTX_ENABLED`                             | `Bool`                                | `false`                      | Publishes the active trace and span identifiers through the OpenTelemetry Thread Context interface, allowing compatible out-of-process tools to correlate profiles with traces. This feature is supported only on Linux when the `libdatadog` native extension is available; enabling it on unsupported platforms has no effect.                                                                                                                                                                                                                                                             |
