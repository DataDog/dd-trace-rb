...
| `tracing.http_client_resource_name_quantize`           | `DD_TRACE_HTTP_CLIENT_RESOURCE_NAME_QUANTIZE`           | `Bool`                                | `false`                      | `true` to append the request path to the resource name of HTTP client spans, e.g. `GET /users/?` instead of `GET`. Path segments that look like identifiers are replaced with `?`. Currently applies to the `net/http` integration.                                                                                                                                                                                                                                                                                                                                                                                                     |
