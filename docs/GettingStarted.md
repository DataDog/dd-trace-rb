...
| `DD_DYNAMIC_INSTRUMENTATION_EVALUATION_TIMEOUT_MS` | `Integer` | Wall-time budget (in milliseconds) for evaluating a probe condition or template segment; an over-budget evaluation is aborted and reported as an evaluation error rather than producing a captured snapshot. | `50` |
...
| `c.dynamic_instrumentation.max_time_to_evaluate_ms` | `Integer` | Wall-time budget (in milliseconds) for evaluating a probe condition or template segment; an over-budget evaluation is aborted and reported as an evaluation error rather than producing a captured snapshot. | `50` |
