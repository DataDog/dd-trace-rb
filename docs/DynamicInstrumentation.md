...
**Note on method parameters:** Parameters are only extracted for
hand-written methods. Methods generated by `attr_writer`,
`attr_accessor`, and ActiveRecord attribute writers do not expose
parameter names through Ruby's introspection APIs. These methods still
appear in autocomplete, but their parameters will not be shown.
