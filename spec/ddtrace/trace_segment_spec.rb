...
RSpec.describe Datadog::TraceSegment do
  ...
  describe '#sampled?' do
    ...
    context 'when sampling priority is set to AUTO_KEEP' do
      before { trace_segment.sampling_priority = Datadog::Ext::Priority::AUTO_KEEP }

      it { is_expected.to be true }
    end

    context 'when sampling priority is set to USER_KEEP' do
      before { trace_segment.sampling_priority = Datadog::Ext::Priority::USER_KEEP }

      it { is_expected.to be true }
    end

    context 'when sampling priority is set to AUTO_REJECT' do
      before { trace_segment.sampling_priority = Datadog::Ext::Priority::AUTO_REJECT }

      it { is_expected.to be false }
    end

    context 'when sampling priority is set to USER_REJECT' do
      before { trace_segment.sampling_priority = Datadog::Ext::Priority::USER_REJECT }
