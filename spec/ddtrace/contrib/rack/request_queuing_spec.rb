...
RSpec.describe 'Rack integration request queuing' do
  ...
  context 'when enabled' do
    let(:rack_options) { super().merge(request_queuing: true) }
    ...
    context 'and a request is received' do
      ...
      context 'with X-Request-Start header' do
        ...
        context 'given a custom web service name' do
          let(:rack_options) { super().merge(web_service_name: web_service_name) }
  ...
  context 'when disabled' do
    let(:rack_options) { super().merge(request_queuing: false) }
