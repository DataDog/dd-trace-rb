...
RSpec.describe Datadog::Tracing::Configuration::Settings do
  ...
  describe '#tracing' do
    ...
    describe '#trace_id_128_bit_propagation_enabled' do
      subject { settings.tracing.trace_id_128_bit_propagation_enabled }

      context 'when given environment variable `DD_TRACE_128_BIT_TRACEID_PROPAGATION_ENABLED' do
        around do |example|
          ClimateControl.modify(
            'DD_TRACE_128_BIT_TRACEID_PROPAGATION_ENABLED' => env_var
          ) do
            example.run
          end
        end
