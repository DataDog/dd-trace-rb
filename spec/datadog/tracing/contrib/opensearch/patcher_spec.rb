...
require 'datadog/tracing/contrib/opensearch/support/client'

RSpec.describe Datadog::Tracing::Contrib::Opensearch::Patcher do
  ...
  describe 'cluster health request' do
    ...
    describe 'health request span' do
      ...
      it { expect(span.name).to eq('opensearch.query') }
      it { expect(span.service).to eq('opensearch') }
      it { expect(span.resource).to eq('GET _cluster/health') }
      it { expect(span.span_type).to eq('opensearch') }
  ...
  describe 'indexing request' do
    ...
    describe 'index request span' do
      ...
      it_behaves_like 'analytics for integration' do
        let(:analytics_enabled_var) { Datadog::Tracing::Contrib::Opensearch::Ext::ENV_ANALYTICS_ENABLED }
        let(:analytics_sample_rate_var) { Datadog::Tracing::Contrib::Opensearch::Ext::ENV_ANALYTICS_SAMPLE_RATE }
      end
      ...
      it 'tags span with quantized request body' do
        expect(span.get_tag('opensearch.body'))
