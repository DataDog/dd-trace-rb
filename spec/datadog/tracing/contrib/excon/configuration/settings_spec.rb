...
RSpec.describe Datadog::Tracing::Contrib::Excon::Configuration::Settings do
  ...
  it_behaves_like 'with error_status_codes setting', env: 'DD_TRACE_EXCON_ERROR_STATUS_CODES', default: 400...600, settings_class: described_class, option: :error_status_codes
  ...
  describe '#distributed_tracing' do
    ...
    context 'when configured via environment variable' do
      around do |example|
        ClimateControl.modify('DD_TRACE_EXCON_DISTRIBUTED_TRACING' => 'false') do
          example.run
        end
      end
