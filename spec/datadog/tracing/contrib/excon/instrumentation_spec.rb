...
RSpec.describe Datadog::Tracing::Contrib::Excon::Middleware do
  ...
  context 'when there is custom error handling' do
    ...
    after { Datadog::Tracing.configuration.tracing[:excon][:error_handler] = nil }
  ...
  context 'when split by domain' do
    ...
    after { Datadog::Tracing.configuration.tracing[:excon][:split_by_domain] = false }
  ...
  context 'when distributed tracing is disabled' do
    ...
    after { Datadog::Tracing.configuration.tracing[:excon][:distributed_tracing] = true }
  ...
  context 'global service name' do
    ...
    before do
      @old_service_name = Datadog::Tracing.configuration.tracing[:excon][:service_name]
      Datadog::Tracing.configure { |c| c.tracing.instrument :excon, service_name: service_name }
    end

    after { Datadog::Tracing.configure { |c| c.tracing.instrument :excon, service_name: @old_service_name } }
