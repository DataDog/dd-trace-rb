...
RSpec.describe Datadog::Tracing::Contrib::ActiveStorage::Events::Download do
  describe '.event_name' do
    it 'returns the correct event name' do
      expect(described_class.event_name).to eq('service_download.active_storage')
    end
  end

  describe '.span_name' do
    it 'returns the correct span name' do
      expect(described_class.span_name).to eq('active_storage.download')
    end
  end

  describe '.process' do
    let(:span) { Datadog::Tracing::SpanOperation.new('active_storage.download') }
    ...
    it 'sets the span resource' do
      ...
      expect(span.resource).to eq('S3: documents/file.pdf')
    end
