...
RSpec.describe 'Transport::HTTP benchmarks' do
  ...
  describe '#send' do
    ...
    it do
      Benchmark.bm do |x|
        x.report('Default Datadog::Tracing::Transport::HTTP') do
          iterations.times do
            default_transport.send_traces(traces)
          end
        end

        x.report('Datadog::Tracing::Transport::HTTP with Net::HTTP adapter') do
          iterations.times do
        ...
        # x.report('Datadog::Tracing::Transport::HTTP with Unix socket adapter') do
        #   iterations.times do
        #     unix_transport.send_traces(traces)
        #   end
        # end

        x.report('Datadog::Tracing::Transport::HTTP with test adapter') do
          iterations.times do
