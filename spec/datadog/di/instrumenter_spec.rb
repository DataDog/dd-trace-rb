...
RSpec.describe Datadog::DI::Instrumenter do
  ...
  describe '.hook_method' do
    ...
    context 'circuit breaker' do
      ...
      let(:responder) do
        # Create a custom responder that tracks all callbacks
        Class.new do
      ...
      context 'when max_processing_time is nil' do
        before do
          allow(di_internal_settings).to receive(:max_processing_time).and_return(nil)
        end

        it 'does not disable probe even if processing is slow' do
          instrumenter.hook_method(probe, responder)

          # Even with real timing, circuit breaker should not engage when disabled
      ...
      context 'when max_processing_time is set' do
        let(:max_processing_time) { 0.1 }

        before do
          allow(di_internal_settings).to receive(:max_processing_time).and_return(max_processing_time)
        end

        context 'when processing time is under threshold' do
          it 'does not disable probe' do
            # Mock fast processing
            call_count = 0
            allow(Process).to receive(:clock_gettime) do |clock_type|
              if clock_type == Process::CLOCK_MONOTONIC
                call_count += 1
                case call_count
                when 1 then 0.0    # di_start_time
                when 2 then 0.0    # start_time (before customer method)
                when 3 then 0.005  # di_duration calculation before customer method
                when 4 then 0.01   # end_time (after customer method)
                when 5 then 0.02   # final check
                else 0.03
                end
              else
                Process.clock_gettime(clock_type)
              end
            end
        ...
        context 'when processing time exceeds threshold' do
          let(:max_processing_time) { 0 }  # Zero threshold means circuit breaker always triggers

          it 'disables probe and calls disabled callback' do
            instrumenter.hook_method(probe, responder)
          ...
          it 'does not execute probe after it is disabled' do
            instrumenter.hook_method(probe, responder)

            # First call should execute and disable the probe
            HookTestClass.new.hook_test_method
            expect(observed_calls.length).to eq 1
            expect(disabled_calls.length).to eq 1
            expect(probe.enabled?).to be false

            # Second call should not execute because probe is disabled
            result = HookTestClass.new.hook_test_method
            expect(result).to eq 42  # Method still works
            expect(observed_calls.length).to eq 1  # Still 1, not 2
            expect(disabled_calls.length).to eq 1  # Still 1, not 2
          end
        end

        context 'when processing time exactly equals threshold' do
          it 'does not disable probe' do
            # Mock processing time exactly at threshold
            call_count = 0
            allow(Process).to receive(:clock_gettime) do |clock_type|
              if clock_type == Process::CLOCK_MONOTONIC
                call_count += 1
                case call_count
                when 1 then 0.0    # di_start_time
                when 2 then 0.0    # start_time
                when 3 then 0.05   # di_duration calculation
                when 4 then 0.05   # end_time
                when 5 then 0.1    # final check - total = 0.05 + (0.1-0.05) = 0.1 (exactly at threshold)
                else 0.2
                end
              else
                Process.clock_gettime(clock_type)
        ...
        context 'when condition evaluation is slow' do
          let(:max_processing_time) { 0 }  # Zero threshold to always trigger
          ...
          it 'includes condition evaluation time in total DI time' do
            instrumenter.hook_method(probe, responder)

            HookTestClass.new.hook_test_method_with_arg(42)
    ...
    context 'circuit breaker' do
      ...
      context 'when max_processing_time is nil' do
        ...
        it 'does not disable probe even if processing is slow' do
          ...
          allow(Process).to receive(:clock_gettime).with(Process::CLOCK_MONOTONIC).and_return(0.0, 10.0)
          ...
          it 'does not disable probe' do
            ...
            allow(Process).to receive(:clock_gettime) do |clock_type|
              if clock_type == Process::CLOCK_MONOTONIC
                ...
                case call_count
                when 1 then 0.0    # di_start_time
                when 2 then 0.1    # final check (exactly at threshold)
                else 0.2
                end
              else
                Process.clock_gettime(clock_type)
