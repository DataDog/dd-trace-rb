...
RSpec.describe Datadog::AppSec::Instrumentation::Gateway do
  ...
  context 'watch' do
    it 'stores middleware' do
      expect(subject.middlewares["hello"]).to be_empty

      subject.watch("hello", "world") do
        1 + 1
      end

      expect(subject.middlewares["hello"]).to_not be_empty
    end
    ...
    it 'doesn\'t stores middleware for previously registered name/key combination' do
      ...
      stored_event = subject.middlewares["hello"]

      subject.watch("hello", "world") do
        2 + 2
      end

      expect(subject.middlewares["hello"]).to eq(stored_event)
    end
    ...
    it 'stores multiple middlewares for disticnt name/key combination' do
      ...
      subject.watch("hello", "world2") do
        2 + 2
      end

      expect(subject.middlewares["hello"].length).to eq(2)
    end
  end

  context 'push' do
    it 'returns provided block if no middleware present' do
      block_result, opts = subject.push("hello", {}) do
        1 + 1
      end
    ...
    it 'wrap provided middlewares on top provided block' do
      ...
      subject.watch("hello", "world") do |_next, env|
        env_dup = env.dup
        env_1 = env_dup
        env[:c] = :d
        _next.call(env)
      ...
      subject.watch("hello", "world2") do |_next, env|
        env_2 = env
        _next.call(env)
      end

      result = subject.push("hello", {a: :b}) do
        [1 +1, :done]
      end
