...
RSpec.describe ReleaseNotes, webmock: true do
  describe ".create_draft_release" do
    ...
    let(:list_releases_url) { "https://api.github.com/repos/DataDog/dd-trace-rb/releases?per_page=100" }
    ...
    before do
      stub_request(:get, list_releases_url).to_return(status: 200, body: "[]")
    end
    ...
    it "fails loudly when listing releases to check for an existing draft errors" do
      stub_request(:get, list_releases_url).to_return(status: 500, body: "server error")

      expect { described_class.create_draft_release(version, "* Fix a bug.") }
        .to raise_error(SystemExit)
    end

    it "fails loudly instead of creating a duplicate when a draft release for the tag already exists" do
      stub_request(:get, list_releases_url).to_return(
        status: 200,
        body: [{"tag_name" => "v2.36.0", "draft" => true}].to_json,
      )
      post_stub = stub_request(:post, releases_url).to_return(status: 201, body: "{}")

      expect { described_class.create_draft_release(version, "* Fix a bug.") }
        .to raise_error(SystemExit)
      expect(post_stub).not_to have_been_requested
    end

    it "creates the draft when an existing release for the tag is not a draft" do
      stub_request(:get, list_releases_url).to_return(
        status: 200,
        body: [{"tag_name" => "v2.36.0", "draft" => false}].to_json,
      )
      stub = stub_request(:post, releases_url)
        .with(body: hash_including("tag_name" => "v2.36.0"))
        .to_return(status: 201, body: "{}")
